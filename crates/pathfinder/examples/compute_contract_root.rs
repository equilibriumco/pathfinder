...
fn main() -> anyhow::Result<()> {
    ...
    let database_path = std::env::args()
        .nth(1)
        .context("missing database path (arg 1)")?;
    ...
    let block_number = std::env::args()
        .nth(2)
        .context("missing block number (arg 2)")?;
    let block_number: u64 = block_number
        .parse()
        .context("Failed to parse block number as u64")?;
    let block_number = BlockNumber::new_or_panic(block_number);

    let contract_address = std::env::args()
        .nth(3)
        .context("missing contract address (arg 3)")?;
    ...
    let storage_address = std::env::args()
        .nth(4)
        .context("missing storage address (arg 4)")?;
    let storage_address =
    ...
    let storage_value = std::env::args()
        .nth(5)
        .context("missing storage value (arg 5)")?;
