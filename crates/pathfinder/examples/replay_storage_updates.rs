...
fn main() -> anyhow::Result<()> {
    ...
    let output_storage = StorageBuilder::file(output_database_path.into())
        .migrate()
        .context("Migrating database")?
        .create_pool(NonZeroU32::new(32).expect("32 is nonzero"))
        .context("Creating connection pool")?;
        ...
        for class_hash in &state_update.declared_cairo_classes {
            output_txn
                .insert_cairo_class_definition(
                    *class_hash,
                    &pathfinder_common::class_definition::SerializedCairoDefinition::default(),
                )
        ...
        for (class_hash, casm_hash) in &state_update.declared_sierra_classes {
            output_txn
                .insert_sierra_class_definition(
                    ...
                    &pathfinder_common::class_definition::SerializedSierraDefinition::default(),
                    &pathfinder_common::class_definition::SerializedCasmDefinition::default(),
                    casm_hash,
                )
