...
pub trait RowExt {
    ...
    fn get_trie_storage_index<Index: RowIndex>(
        ...
        let idx = self.get_i64(index)?;
        // TODO fix TrieStorageIndex to use i64 internally, just like the BlockNumber
        // does
        Ok(TrieStorageIndex(u64::try_from(idx).expect(
            "TrieStorageIndex is non-negative and within i64::MAX",
        )))
    }
    ...
    fn get_optional_trie_storage_index<Index: RowIndex>(
        ...
        let idx = self.get_optional_i64(index)?;
        // TODO fix TrieStorageIndex to use i64 internally, just like the BlockNumber
        // does
        Ok(idx.map(|idx| {
            TrieStorageIndex(
                u64::try_from(idx).expect("TrieStorageIndex is non-negative and within i64::MAX"),
            )
        }))
    }
