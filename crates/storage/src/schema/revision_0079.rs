...
pub(crate) fn migrate(
    ...
    // otherwise discard the migration's RocksDB output and force a full replay
    // on next boot, which is hours of work on a synced mainnet chain.
    // `flush()` alone only covers the default CF, hence the explicit list.
    tracing::info!("Flushing RocksDB memtables after schema revision 0079");
    let flushed_cfs = [
        rocksdb.get_column(&STATE_UPDATES_COLUMN),
        rocksdb.get_column(&NONCE_UPDATES_COLUMN),
        rocksdb.get_column(&STORAGE_UPDATES_COLUMN),
        rocksdb.get_column(&TRANSACTIONS_AND_RECEIPTS_COLUMN),
        rocksdb.get_column(&TRANSACTION_HASHES_COLUMN),
        rocksdb.get_column(&crate::connection::EVENTS_COLUMN),
    ];
    rocksdb
        .rocksdb
        .flush_cfs_opt(
        ...
        .context("Flushing RocksDB memtables after revision 0079 migration")?;
...
impl<'stmt> PeekableBlocks<'stmt> {
    ...
    fn next_block(&mut self) -> anyhow::Result<Option<u64>> {
        match self.rows.next()? {
            Some(row) => Ok(Some(row.get_u64(0)?)),
            None => Ok(None),
        }
    }
}
...
fn migrate_transaction_hashes(
    ...
    let rows = stmt
        .query_map([], |row| {
            ...
            let block_number = row.get_u64(1)?;
...
fn migrate_events(
    ...
    for row in rows {
        ...
        if written.is_multiple_of(EVENTS_BATCH_SIZE as u64) {
            rocksdb
                .rocksdb
                .write_without_wal(&batch)
                .context("Writing events batch to RocksDB")?;
            batch = crate::RocksDBBatch::default();
            tracing::info!("Migrated {written} events entries");
        }
    }
...
mod tests {
    ...
    /// `revision_0079::migrate` must flush the memtables of every column
    ...
    #[test]
    fn revision_0079_migrate_flushes_memtable() {
        let (mut conn, rocksdb, _rocksdb_dir) = fresh_pre_0079_db();

        // Seed rows in every SQLite table revision_0079 reads. Without
        // seeding, the migrated CFs would end up empty and the per-CF
        // memtable assertion below would pass vacuously.
        let tx = conn.transaction().unwrap();
        for i in 0u64..8 {
            insert_block_header(&tx, i);
        }
        // Nonces and storage keys the migration joins against.
        ...
        for i in 0u64..8 {
            ...
            tx.execute(
                "INSERT INTO nonce_updates (block_number, contract_address_id, nonce) VALUES (?, \
                 1, ?)",
                params![i as i64, &nonce.to_vec()],
            )
            .unwrap();
            let mut value = [0u8; 32];
            value[16..24].copy_from_slice(&i.to_be_bytes());
            tx.execute(
                "INSERT INTO storage_updates (block_number, contract_address_id, \
                 storage_address_id, storage_value) VALUES (?, 1, 1, ?)",
                params![i as i64, &value.to_vec()],
            )
            .unwrap();
            let mut tx_hash = [0u8; 32];
            tx_hash[24..].copy_from_slice(&i.to_be_bytes());
            tx.execute(
                "INSERT INTO transaction_hashes (hash, block_number, idx) VALUES (?, ?, 0)",
                params![&tx_hash.to_vec(), i as i64],
            )
            .unwrap();
        }
        ...
        for column in [
            &crate::STATE_UPDATES_COLUMN,
            &crate::NONCE_UPDATES_COLUMN,
            &crate::STORAGE_UPDATES_COLUMN,
            &crate::TRANSACTIONS_AND_RECEIPTS_COLUMN,
            &crate::TRANSACTION_HASHES_COLUMN,
            ...
            assert_eq!(
                (active, immut),
                (Some(0), Some(0)),
                "column family {} still had memtable entries after revision_0079::migrate: \
    ...
    fn migrate_transaction_hashes_targets_transaction_hashes_column() {
        ...
        tx.execute(
            ...
            params![&hash.0.as_be_bytes().to_vec(), 7i64, 3i64],
        )
        .unwrap();
