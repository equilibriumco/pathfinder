...
fn validate_proposer(
    ...
    // (e.g. reading the proposer set), not a fault in the proposal, so it is
    // fatal rather than a rejected proposal.
...
mod tests {
    ...
    fn test_empty_proposal_finalization() {
        ...
        let validator_transaction_batch = validator_block_info
            .validate_block_info(
                ...
                &ConstantProposer(contract_address!("0x1")),
            )
    ...
    fn validate_block_info_rejects_unexpected_proposer() {
        ...
        // Proposal at height 0 authored by proposer 0x1.
        let proposal_init = p2p_proto::consensus::ProposalInit {
            height: 0,
            round: 0,
            valid_round: None,
            proposer: p2p_proto::common::Address(felt!("0x1")),
            timestamp: 1000,
            builder: p2p_proto::common::Address(felt!("0x1")),
            l1_da_mode: p2p_proto::common::L1DataAvailabilityMode::Calldata,
            l2_gas_price_fri: 1,
            l1_gas_price_fri: 1_000_000_000,
            l1_data_gas_price_fri: 1,
        ...
        // The selector expects a different proposer (0x2), so validation must
        // reject the proposal before any other checks.
        let expected = contract_address!("0x2");
        let actual = contract_address!("0x1");
        let err = validator_block_info
            ...
            .expect_err("Mismatched proposer should be rejected");
